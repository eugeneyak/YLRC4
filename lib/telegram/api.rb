...
module Telegram::API
  ...
  def edit_message_caption(chat_id, message_id, caption, caption_entities: [], show_caption_above_media: false)
    Telegram::Client.instance.post "editMessageCaption", chat_id:, message_id:, caption:, show_caption_above_media:, caption_entities:
  end
  ...
  def set_message_reaction(whom, message_id, reaction = "❤", big: false)
    chat_id =
      case whom
      in Integer           then whom
      in id: Integer => id then id
    ...
    Telegram::Client.instance.post "setMessageReaction",
      chat_id:, message_id:, is_big: big, reaction: Array(reaction).map { { type: "emoji", emoji: it } }
  end
end
